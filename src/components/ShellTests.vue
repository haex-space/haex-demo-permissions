...
const tests = ref<TestResult[]>([
	...
	{
		name: 'Execute blocked command',
		description: 'Run rm -rf / (should fail)',
		expected: 'error',
		status: 'pending',
	},
])
...
const runAllTests = async () => {
	for (const test of tests.value) {
		test.status = 'pending'
		test.message = 'Shell permissions not yet implemented in SDK'
	}
}
...
<template>
	<div>
		<div class="flex justify-between items-center mb-4">
			<h2 class="text-xl font-semibold">Shell Permission Tests</h2>
		...
		<div class="mb-4 p-3 bg-base-200 rounded-lg">
			<p class="text-sm text-warning">
				Shell permissions are not yet implemented in the SDK.
			</p>
		</div>
